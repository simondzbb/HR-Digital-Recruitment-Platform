...
class PDFHandler:
    ...
    def extract_json_from_pdf(self, pdf_path: str) -> Optional[JSONResume]:
        """Backward-compatibility alias for ``pdf_dispatcher.extract_resume_data``.

        Historically the only supported format was PDF; callers (and tests)
        that still invoke ``PDFHandler.extract_json_from_pdf(path)`` continue
        to work because the implementation now defers to the dispatcher,
        which transparently handles PDF, DOCX, DOC, and image inputs.

        The argument is named ``pdf_path`` for backward compatibility but
        accepts any path the dispatcher supports.
        """
        from pdf_dispatcher import extract_resume_data

        try:
            return extract_resume_data(pdf_path)
        except Exception as e:
